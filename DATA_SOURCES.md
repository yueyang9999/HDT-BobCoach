...
当前公开版本事实（2026-07-24 核验）：GitHub 仓库为 public，默认分支为 `main`，唯一公开 Release 为 `v0.2.0-beta.1`。仓库源码的 `1.0.0` 仅是本地 release candidate；未创建 1.0.0 Release，1.0.0 ZIP 和 checksum 不应被写成公开下载地址。
...
- **请求边界：** 若未来适配器经单独审批启用，仅允许 HTTPS GET、无请求体，User-Agent 为 `BobCoach/1.0 external-validation-readonly`；URL 只可包含已知的数字 Build，不发送日志、回放、账号、对手、用户档案或设备标识。
