...
治理登记复核日期：2026-07-21；离线饰品规则复核日期：2026-07-22
...
GitHub 仓库中的代码、文档、规则快照、议题、PR、镜像、fork、评论和外部链接不等于官方数据、完整数据或最新数据。仓库内容可能过期、缺失、被修改或暂时不可用，也可能与用户本机的 HDT/HearthDb build 不一致；它们不自动成为 Bob Coach 的运行时输入、第三方原始响应、授权依据或任何权利人的背书。GitHub 仓库、议题、PR、镜像、fork、评论和外部链接也不自动构成数据源登记中的获批端点。
