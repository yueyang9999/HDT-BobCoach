...
3. 生成的 ZIP 与外部 `BobCoach-<version>-win-x64.zip.sha256` 完整匹配，且没有未跟踪的产物进入 Git。
...
仅在获授权后，上传严格包白名单生成的 ZIP 与 `BobCoach-<version>-win-x64.zip.sha256`。发布说明必须写明版本、兼容基线、已知限制、安装文档、隐私和数据来源链接。不得使用任何第三方 Logo，不得把赞赏与功能、优先支持或游戏/HDT overlay 关联。
