...
$output = Join-Path $env:TEMP ("bobcoach_comp_strategy_exit_" + [Guid]::NewGuid().ToString("N") + ".exe")
$sources = @(
    (Join-Path $root "src\BobCoach\Core\CardSemanticFactSource.cs"),
    (Join-Path $root "src\BobCoach\Core\SemanticSynergyEvaluator.cs"),
    (Join-Path $root "src\BobCoach\Core\SynergyEngine.cs"),
    (Join-Path $PSScriptRoot "CompStrategyExitBehavior.cs")
)
