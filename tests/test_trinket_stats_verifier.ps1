...
$output = Join-Path $env:TEMP "bobcoach_trinket_stats_verifier.exe"
$sources = @(
    (Join-Path $root "src\BobCoach\Core\TrinketStatsModels.cs"),
    (Join-Path $root "src\BobCoach\Core\TrinketStatsVerifier.cs"),
    (Join-Path $PSScriptRoot "TrinketStatsVerifierHarness.cs")
)
