...
$output = Join-Path $env:TEMP "bobcoach_plugin_integrity_verifier.exe"
$sources = @(
    (Join-Path $root "src\BobCoach\Core\PluginIntegrityVerifier.cs"),
    (Join-Path $PSScriptRoot "PluginIntegrityVerifierHarness.cs")
)
